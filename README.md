...
### **Redistricting:** Time estimate: ~15-30 minutes for 150,000 blocks
  - Run seed_grow_districts
  - When prompted give the path to the three column prediction csv file that was generated from our model.
  - When prompted again give the folder path with all of the testing data
    - There's already a folder in github with the data called 'testing data'
  - Then for each state you are asked how many districts and the number of those for each party.
  - The output is displayed in the console as well as creating a csv file called (name_of_prediction_file)_seed_grow_districts.csv
### **Visualizing the districts:** Time estimate: ~15-30 minutes for 150,000 blocks
  - Needs python -m pip install pillow
  - When prompted, give the file path to the redistricting file that was generated by seed_grow_districts.py
  - Output photo will generate in the district_maps folder.
